...
const AvengersSection = () => {
  ...
  return (
    <section className="py-8 md:py-12 w-full bg-[#f4f4f5]">
      <SectionContainer>
        {/* Rounded white card container — matches the reference */}
        <div className="bg-white rounded-[28px] p-8 md:p-12 lg:p-16">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8">
            
            {/* Left Column */}
            <div className="lg:col-span-5 flex flex-col pr-0 lg:pr-8">
              <h4 className="text-[10px] font-bold tracking-widest uppercase text-black/50 mb-6">OUR AVENGERS</h4>
              
              <h2 className="text-[32px] md:text-[40px] lg:text-[48px] font-heading font-medium text-[#111] leading-[1.15] tracking-tight mb-8">
                Meet with our team member
              </h2>

              {/* Tabs */}
              <div className="flex items-center gap-8 mb-8 border-b border-black/10 pb-3 w-full">
                {['DESIGN TEAM', 'DEVELOPMENT TEAM'].map(tab => (
                  <button
                    key={tab}
                    onClick={() => setActiveTab(tab)}
                    className={`text-[11px] font-bold tracking-widest transition-colors relative pb-3 -mb-3 ${
                      activeTab === tab
              ...
              <p className="text-[13px] md:text-[14px] leading-[1.8] text-black/50 font-medium mb-10 max-w-[380px]">
              ...
              {/* Join Us Button */}
              <div className="flex items-center gap-4 mb-12 cursor-pointer group w-max">
              ...
              {/* Bottom Group Photo */}
              <div className="w-full rounded-[20px] overflow-hidden aspect-16/10">
            ...
            <div className="lg:col-span-7 lg:col-start-6">
              <div className="grid grid-cols-2 gap-4 md:gap-6">
                {currentTeam.map((member, i) => (
                  <div key={i} className="bg-[#f6f6f6] rounded-[20px] overflow-hidden flex flex-col p-3 group">

                    {/* Member Image */}
                    <div className="w-full aspect-4/5 bg-[#cbb99f] rounded-[16px] overflow-hidden mb-4">
                      <img
                        src={member.image}
                        alt={member.name}
                    ...
                    <div className="flex flex-col px-1 pb-1">
                      <h3 className="text-[14px] md:text-[16px] font-heading font-semibold text-[#111] tracking-tight leading-tight mb-0.5">
                        {member.name}
                      </h3>
                      <p className="text-[9px] uppercase tracking-widest font-bold text-black/30 mb-4">
                        {member.role}
                      </p>
