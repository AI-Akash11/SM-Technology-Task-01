...
const AvengersCards = ({ activeTab }) => {
  ...
  const developmentTeam = [
    ...
    },
    {
      ...
      role: "FOUNDER",
      image: "/avengers-img-5.png",
    },
  ];

  const currentTeam =
    activeTab === "DESIGN TEAM" ? designTeam : developmentTeam;
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 animate-fadeIn">
      {currentTeam.map((member, index) => (
        <div
          key={index}
          className="group relative bg-[#EDEDED] rounded-[24px] overflow-hidden p-6 md:p-8 flex flex-col items-center shadow-lg hover:shadow-2xl transition-all duration-700 hover:-translate-y-2 cursor-pointer"
        >
          ...
          <div className="absolute top-6 right-6 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
            <div className="w-8 h-8 rounded-full bg-black text-white flex items-center justify-center text-[10px] hover:scale-110 transition-transform cursor-pointer">
              In
            </div>
            <div className="w-8 h-8 rounded-full bg-black text-white flex items-center justify-center text-[10px] hover:scale-110 transition-transform cursor-pointer">
              X
